// LONGEST-CHAIN sweep test, plaintext space p = 2^16 + 1 = 65537 (17 bits).
//
// Smaller plaintext space → smaller noise growth per op → smaller chain → smaller initial budget.
//
// Structure of the N_OPS BGV ops:
//   - op 1:           F_p × ct1
//   - op 2:           ct1 × ct1 + relin
//   - ops 3..N_OPS:   F_p × ct1   (plain multiplications)
// Mod-switch every 2 ops. No smudge (no ZK). No step 5 (in the original 5-step pipeline sense):
// the original "step 5" plain mul is folded into the parametric ops 3..N_OPS.
//
// Reports SEAL's invariant_noise_budget BEFORE and AFTER each op (so you can see exactly how
// many bits of budget each operation burns).
//
// Set N_OPS via env var. Defaults to 4.
// Set UPPER_BITS via env var. Defaults to 17 (the smallest prime SEAL can find ≡ 1 mod 2N=16384).
...
int main() {
    const size_t N = 8192;
    uint64_t p_val = (uint64_t(1) << 16) + 1;          // 65537 (17 bits)
    if (const char* e = getenv("P_VAL")) p_val = strtoull(e, nullptr, 10);
    ...
    // Defaults to 3. With mod-switch every 2 ops, even N_OPS needs N_OPS/2 mod-switches and
    // odd N_OPS needs (N_OPS-1)/2 mod-switches (last op runs alone at L=3).
    int n_ops = 3;
    if (const char* e = getenv("N_OPS")) n_ops = atoi(e);
    if (n_ops < 2) n_ops = 2;

    // For odd N_OPS, the lone last op runs at L=3 (no further mod-switch needed).
    // n_upper_primes = number of mod-switches needed in the BGV stage.
    int n_upper_primes;
    if (n_ops % 2 == 0) n_upper_primes = n_ops / 2;
    else n_upper_primes = (n_ops - 1) / 2;
    const int L_top = n_upper_primes + 3;

    // Upper-prime size: defaults to 17 (smallest SEAL can find for N=8192).
    int upper_bits = 17;
    if (const char* e = getenv("UPPER_BITS")) upper_bits = atoi(e);

    // Build coeff_modulus.
    vector<int> chain_bits = {(int)q0_bits, q1_bits, q2_bits};
    for (int i = 0; i < n_upper_primes; i++) chain_bits.push_back(upper_bits);
    ...
    // ---- Encrypt ct1 with μ = constant 1 polynomial (so ct1 plain starts at 1) ----
    Plaintext mu1_pt("1");
    Ciphertext ct1;
    ...
    // For plain-mul steps, use a RANDOM plaintext (in coefficient form, values in [0, p)).
    // This gives a realistic ‖m‖_canonical and therefore realistic per-op noise burn.
    // All plain muls (op 1, ops 3..N_OPS) use the SAME m so the final ct1 plain = m^N_OPS,
    // which we compute plaintext-side for the mkDec verification at the end.
    vector<uint64_t> m_coeffs(N);
    {
        mt19937_64 rng_plain(424242);
        for (size_t i = 0; i < N; i++) m_coeffs[i] = rng_plain() % p_val;
    }
    Plaintext m_pt;
    m_pt.resize(N);
    for (size_t i = 0; i < N; i++) m_pt[i] = m_coeffs[i];
    ...
    // ops 3..N_OPS: plain muls with the random m, mod-switch every 2 ops
    for (int op = 3; op <= n_ops; op++) {
        // Re-encode the same random m at the current level
        Plaintext mp;
        mp.resize(N);
        for (size_t i = 0; i < N; i++) mp[i] = m_coeffs[i];
        eval.transform_to_ntt_inplace(mp, ct1.parms_id());
        t_start = steady_clock::now();
        eval.multiply_plain_inplace(ct1, mp);
        long t = take_us();
        ostringstream oss; oss << "op " << op << "  (F_p × ct_1)";
        print_op(oss.str(), t);

        // Mod-switch after every 2 ops (i.e. after even-numbered ops), unless it's the last op
        // and there's no upper-level prime left to drop.
        if (op % 2 == 0 && ct1.coeff_modulus_size() > 3) {
            t_start = steady_clock::now();
            eval.mod_switch_to_next_inplace(ct1);
            long tm = take_us();
            ostringstream oss2; oss2 << "  + mod-switch (after pair " << (op / 2) << ")";
            print_op(oss2.str(), tm);
        }
    }
    ...
    // --- Compute m^N_OPS plaintext-side for verification.
    // After the chain, ct1's underlying poly = m^N_OPS (1 plain mul + 1 squaring + (N-2) plain muls).
    NTTTables p_ntt(/*log_N=*/13, Modulus(p_val));
    auto plain_mul_p = [&](const vector<uint64_t>& a, const vector<uint64_t>& b) {
        vector<uint64_t> an(a), bn(b), c(N);
        ntt_negacyclic_harvey(an.data(), p_ntt);
        ntt_negacyclic_harvey(bn.data(), p_ntt);
        dyadic_product_coeffmod(an.data(), bn.data(), N, Modulus(p_val), c.data());
        inverse_ntt_negacyclic_harvey(c.data(), p_ntt);
        return c;
    };
    vector<uint64_t> m_power(N, 0);
    {
        // Start: constant 1 polynomial
        m_power[0] = 1;
        // op 1: ct *= m → plain becomes m
        m_power = plain_mul_p(m_power, m_coeffs);     // = m
        // op 2: ct = ct² → plain squares: m²
        m_power = plain_mul_p(m_power, m_power);       // = m²
        // ops 3..N_OPS: each multiplies plain by m
        for (int op = 3; op <= n_ops; op++) {
            m_power = plain_mul_p(m_power, m_coeffs);
        }
    }

    // --- Verify SEAL ct1 decrypts to m^N_OPS ---
    {
        ...
        for (size_t i = 0; i < N; i++) {
            ...
            if (got != m_power[i]) { ok = false; first_bad = i; break; }
        }
        cout << "  [post-step-6 SEAL decrypt check]   " << (ok ? "OK" : "FAIL");
        if (!ok) cout << " (first bad: idx " << first_bad << ", got "
                      << ((first_bad < pt_check.coeff_count()) ? pt_check[first_bad] : 0ULL)
                      << ", want " << m_power[first_bad] << ")";
        cout << "\n\n";
    }
    ...
    // Verifies bridge + sk extraction + mod-switch all work for the scalar case.
    {
        ...
        size_t first_bad = N;
        for (size_t i = 0; i < N; i++) {
            if (dec_msg[i] != m_power[i]) { ok = false; first_bad = i; break; }
        }
    ...
    for (size_t i = 0; i < proto_params.ell_prime; i++) {
        ...
        // ct1 plaintext = m^N_OPS; ct2 slot plaintext = m2_data[i] (or T·m2 for sparsi rows).
        // The joint mkMul plaintext for slot i = m^N_OPS · m2_data[i] (computed plaintext-side).
        vector<uint64_t> ct2_mu_i;
        if (i < proto_params.ell) {
            ...
            for (size_t c = 0; c < proto_params.ell; c++) {
                ...
                ct2_mu_i = mk::plain_add(ct2_mu_i, prod, proto_params);
            }
        }
        // mu_i = m^N_OPS × ct2_mu_i (polynomial product mod p)
        vector<uint64_t> mu_i = plain_mul_p(m_power, ct2_mu_i);
    ...
    // Step 10: mod-switch tensor L=3 → L=2 → L=1.  With intermediate mkDec check.
    {
        ...
        // Intermediate mkDec at L=2 to isolate where it breaks.
        {
            ...
            for (size_t i = 0; i < proto_params.ell; i++) {
                // Expected = K · m^N_OPS · m2_data[i]  mod p
                auto prod = plain_mul_p(m_power, m2_data[i]);
                vector<uint64_t> expected(N);
                for (size_t j = 0; j < N; j++)
                    expected[j] = multiply_uint_mod(prod[j], K_mp, proto_params.p);
    ...
    for (size_t i = 0; i < proto_params.ell; i++) {
        ...
        vector<uint64_t> expected(N);
        for (size_t j = 0; j < N; j++)
            expected[j] = multiply_uint_mod(prod[j], K_mod_p, proto_params.p);
