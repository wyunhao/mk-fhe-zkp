// NON-ZK full pipeline test: same as prototype_full_chain.cpp but WITHOUT the smudge step
// (step 9). Saves the chain bits we'd otherwise reserve to leave room for smudge → smaller
// initial noise budget. Steps 7, 8, 10 still happen. K-scaling and √K Gaussian noise from
// step 8 still added so that 2^20 independent additions are simulated honestly.
//
// Per user's instruction (2026-05-20):
//   - Drop step 3.
//   - Use 49-bit upper primes.
//   - Mod-switch every two ops.
//   - Skip step 9 (no smudge → no ZK).
//
// Smaller chain: K_top = 5 data primes (vs 6 with smudge), since the last internal mod-switch
// can be merged with step 6's mod-switch to L=3 (the no-smudge L=3 budget requirement is less).
//
// Pipeline:
//   1. F_p × ct1                       (no mod-switch yet)
//   2. ct1 × ct1 + relin  → mod-switch (absorbs steps 1 & 2)
//   4. F_p × ct1                       (no mod-switch yet)
//   5. F_p × ct1  → mod-switch         (absorbs steps 4 & 5, lands at L=3)
//   6. final mod-switch to L=3         (no-op)
//   7. mkMul(ct_1, ct_2)
//   8. simulate 2^20 independent additions (K-scaling + √K Gaussian noise)
//   (step 9 SKIPPED — no smudge)
//   10. mod-switch tensor L=3 → L=2 → L=1 (final prime ~34 bits)
...
int main() {
    ...
    // 5 data primes total: q0, q1, q2 at L=3, plus 2 × 49b for upper levels.
    // Mod-switch every 2 ops = 2 internal mod-switches → L=5 → L=3 (step 6 is a no-op).
    // One prime saved vs the smudge version because we don't need the extra L=3 headroom for smudge.
