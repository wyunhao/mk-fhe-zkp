// TIGHTEST chain test (non-ZK, drop step 5 in addition to step 3).
//
// Pipeline now has only 3 BGV ops on ct_1:
//   op 1 (step 1): F_p × ct1
//   op 2 (step 2): ct1 × ct1 + relin    → mod-switch  (absorbs ops 1 & 2)
//   op 3 (step 4): F_p × ct1            (no further mod-switch in BGV stage — already at L=3)
//   step 6:        mod-switch to L=3    (no-op)
//   step 7:        mkMul(ct_1, ct_2)
//   step 8:        simulate 2^20 independent additions (K-scaling + √K Gaussian)
//   (step 9 SKIPPED — no smudge)
//   step 10:       mod-switch tensor L=3 → L=2 → L=1 (final prime ~34 bits)
//
// Chain: K_top = 4 data primes ({q0=34, q1=49, q2=47, upper=U}). Initial budget ≈ chain−24.
// We sweep U (upper prime size) via env UPPER_BITS to find the minimum U that still decrypts.
//
// Default U = 49 (matches the previous step-by-step variant).
...
int main() {
    ...
    // 4 data primes total: {q0, q1, q2} at L=3, plus ONE upper prime of `upper_bits`.
    // Mod-switch happens once (after op 2). Op 3 (the only post-mod-switch BGV op) runs at L=3.
    int upper_bits = 49;
    ...
    // Tight chain (steps 3 and 5 dropped):
    //   op 1 (step 1): F_p × ct_1                       (no mod-switch yet)
    //   op 2 (step 2): ct_1 × ct_1 + relin → mod-switch (absorbs ops 1 & 2; lands at L=3)
    //   op 3 (step 4): F_p × ct_1                       (no further mod-switch)
    //   step 6: mod-switch to L=3 — no-op.
    ...
    cout << "  op 1 (step 1, F_p × ct_1)            " << fmt(t1) << "       " << b1 << "\n";
    ...
    cout << "  op 2a (step 2, ct_1 × ct_1)          " << fmt(t2) << "       " << b2 << "\n";
    ...
    // op 3 (was step 4): plain mul at L=3, no further mod-switch.
    {
        Plaintext mp("1");
        ...
        cout << "  op 3 (step 4, F_p × ct_1)            " << fmt(t) << "       " << b << "\n";
    }
