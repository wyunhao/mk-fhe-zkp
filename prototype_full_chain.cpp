// Full pipeline test:
//   Steps 1-6 are standard BGV operations on ct_1 (driven through SEAL — has
//   tight noise-budget tracking via Decryptor::invariant_noise_budget).
//   Steps 7-10 are multi-key + smudge + mod-switch on the joint (ct_1, ct_2) tensor,
//   implemented in our prototype framework.
//
// Pipeline:
//   1. F_p × ct1          → mod-switch
//   2. ct1 × ct1 + relin  → mod-switch
//   3. F_p × ct1          → mod-switch
//   4. F_p × ct1          → mod-switch
//   5. F_p × ct1          → mod-switch
//   6. Final mod-switch to L=3 (matches ct_2's modulus)  [no-op if already there]
//   7. mkMul(ct_1, ct_2)
//   8. simulate 2^20 independent additions
//   9. smudge noise +40 bits
//   10. mod-switch tensor L=3 → L=2 → L=1 (final prime ~33 bits)
...
int main() {
    ...
    // 6 data primes total: q0, q1, q2 at L=3, plus 3 × 49b for upper levels.
    // Mod-switch happens every 2 ops, so 2 internal + 1 final = 3 mod-switches → drops 3 primes
    // from L=6 → L=3.
    parms.set_coeff_modulus(CoeffModulus::Create(N, {(int)q0_bits, q1_bits, q2_bits,
                                                     49, 49, 49, 60}));
    ...
    // New chain (per user request): drop step 3, mod-switch every 2 ops.
    //   Step 1: F_p × ct_1         (no mod-switch yet)
    //   Step 2: ct_1 × ct_1 + relin → mod-switch  (absorbs steps 1 & 2)
    //   Step 4: F_p × ct_1         (no mod-switch yet)
    //   Step 5: F_p × ct_1 → mod-switch  (absorbs steps 4 & 5)
    //   Step 6: final mod-switch to L=3
    ...
    // Step 4: plain mul, no mod-switch
    {
        ...
        cout << "  Step 4 (F_p × ct_1)                   " << fmt(t) << "       " << b << "\n";
    }

    // Step 5: plain mul, then mod-switch (absorbs steps 4 & 5)
    {
        Plaintext mp("1");
        eval.transform_to_ntt_inplace(mp, ct1.parms_id());
        t_start = steady_clock::now();
        ...
        long t = take_us();
        int b = dec.invariant_noise_budget(ct1);
        cout << "  Step 5 (F_p × ct_1)                   " << fmt(t) << "       " << b << "\n";
    }
    t_start = steady_clock::now();
    eval.mod_switch_to_next_inplace(ct1);
    long t5m = take_us();
    int b5m = dec.invariant_noise_budget(ct1);
    cout << "    + mod-switch (after step 5)         " << fmt(t5m) << "       " << b5m << "\n";

    // Step 6: final mod-switch to land at L=3 if not already there.
    {
    ...
    // Step 9: smudge by +40 bits of total noise.
    //   Pre-smudge noise inf-norm ≈ σ_K · √(2 log N) ≈ 2^44.
    //   Target: post-smudge ≈ 2^(44+40) = 2^84. Limited by uniform_int_distribution<int64_t>: cap 60.
    //   Effective smudge growth on top of existing noise = min(40, 60 - pre_log2) bits.
    {
        double pre_log2 = std::log2(sigma_K * std::sqrt(2.0 * std::log((double)N)));
        int smudge_log2 = (int)round(pre_log2 + 40.0);
        if (smudge_log2 > 60) smudge_log2 = 60;
        t_start = steady_clock::now();
        if (!SKIP_STEP_9) {
            for (auto& d : tct.d3) mk::add_smudge_poly(d, smudge_log2, proto_params, prng_proto);
        }
        long t9 = take_us();
        double eff_growth = (double)smudge_log2 - pre_log2;
        cout << "  Step 9 (smudge → 2^" << smudge_log2 << ", +"
             << fixed << setprecision(1) << eff_growth << "b vs pre)      "
             << fmt(t9) << "       ~" << smudge_log2 << "\n";
    }
